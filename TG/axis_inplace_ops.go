...
func (A *Tensor) Axis_InplaceOperation(axis int, op Inplace_Operation) *Tensor {
	...
	A_Transpose := A.Transpose(axisReordering)
	...
	result := Batch_Tensor_Tensor_Operation(batchOp, A_Transpose)

	// Transpose the result back to the original shape
	resultTranspose := result.Transpose(Permute_Shape(result.Shape, 0, axis))
...
func (b *BatchInplaceOperation) Execute(tensor *Tensor) *Tensor {
	// Apply the Inplace_Operation to the tensor
	b.op.Apply_InplaceOp(tensor)
	return tensor
}
