...
- Go-LinAlg is the root directory of the project. It contains various documentation for the project. 

- Go-LinAlg/GLA contains the source files for the library itself. This is where all of the functions are defined.

- Go-LinAlg/Scripts contains various scripts for the project. 

- Go-LinAlg/Tests contains the tests for the library. Each source file has a corresponding test file.

- Go-LinAlg/Examples contains examples of how to use the library (currently under construction)


# Source File Structure

This next section describes the order you should familiarlize yourself with the source files in the GLA directory.

## tensor.go

Within the GLA directory, there are certain core files that impact the way the entire library is structured. The most important file of this nature, and the one you should look over first if you're interested in contributing to this project is [tensor.go](GLA/tensor.go)

GLA is a library intended for working with arrays (Tensors) of arbitrary dimmension. [tensor.go](GLA/tensor.go) is where the Tensor struct is defined. The Tensor struct is the core data structure of the library. It is a struct that contains a slice of floats, and a slice of ints that describe the multi-dimmensional shape of the tensor. 
...
How does batching work? The [batching.go](GLA/batching.go) files contains a variety of functions that generalize the process of performing an operation on a batch of Tensors of any number of elements. 
...
## Tensor Operations ----> all_element_ops.go, all_inplace_ops.go, axis_element_ops.go, axis_inplace_ops.go, vector.go, matrix.go, elimination.go
...
- If element_ follows axis_, then the operation collapses the axis in which the operation is perforemd. This can be thought of as treating each element along the axis as its own tensor. The colapse of the axis is then the result of performing an operation that results in a scalar, thus 'collapsing' that axis into the scalar.
...
## extentensions.go
Some operations utilize remote repositories that are cloned into the Go-LinAlg/Extensions directory. The extentions.go file directs scripts in the Go-LinAlg/Scripts directory to clone these repositories. At current, this aspect of the library is still under construction and thus there is limited documentation on how to use it.
